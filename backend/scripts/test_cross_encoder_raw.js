...
async function test() {
  ...
  const inputs1 = await tokenizer(archaicText, { text_pair: modernSummary, padding: true, truncation: true });
  const { logits: logits1 } = await model(inputs1);
  console.log("Valid logits:", logits1.data);
  
  const inputs2 = await tokenizer(archaicText, { text_pair: hallucinatedSummary, padding: true, truncation: true });
